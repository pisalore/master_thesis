...
def parse_doc(pdf_path, xml_path, annotations_path, debug):
                ...
                if isinstance(element, LTTextBoxHorizontal):
                    ...
                    if page_layout.pageid == 1:
                        ...
                        if title_font.get("font_size") < float(element_font_size):
                            ...
                            title_font["coords"] = calc_coords_from_pdfminer(
                                [element.bbox]
                            )[0]
                        if (
                            tei.title
                            and doc_instances.get("title")
                            and are_similar(element.get_text(), tei.title)
                        ):
                            doc_instances["title"] = {
                                "content": tei.title,
                                "coords": coords,
                            ...
                            tei.authors
                            and element_contains_authors(
                                tei.authors, element.get_text()
                            )
                            ...
                            if tei.abstract and are_similar(
                                element.get_text(), tei.abstract, "Abstract"
                            ):
                                doc_instances["abstract"] = {
                                    "content": tei.abstract,
                                    "coords": coords,
                                }
                            ...
                            tei.keywords
                            and check_keyword(element.get_text(), tei.keywords)
                    ...
                    if not check_subtitles(
                        element.get_text(), tei.subtitles.get("titles_contents")
                    ):
                        ...
                        if (
                            (
                                ...
                                and not (in_keywords or in_authors)
                            )
                            and page_layout.pageid == 1
                            ...
                            if doc_instances.get("formulas").get(page_layout.pageid):
                                ...
                                for f in doc_instances.get("formulas").get(
                                    page_layout.pageid
                                ):
                                    if do_overlap(f.get("coords"), coords):
                                        adjust_overlapping_coordinates(
                                            f.get("coords"), coords, 20
                                        )
                            if page_layout.pageid == 1:
                                if doc_instances.get("abstract").get(
                                    "coords"
                                ) and do_overlap(
                                    doc_instances.get("abstract").get("coords"), coords
                                ):
    ...
    if doc_instances["authors"]["coords"]:
        doc_instances["authors"]["coords"] = calc_coords_from_pdfminer(
            doc_instances["authors"]["coords"]
        )[0]
    ...
    for table_page in doc_instances["tables"].keys():
        ...
        if doc_instances["tables"].get(table_page):
            doc_instances["tables"][table_page] = [
                t for t in doc_instances["tables"][table_page] if t not in not_tables
            ]
        if doc_instances["text"].get(table_page):
            doc_instances["text"][table_page] = [
                t for t in doc_instances["text"][table_page] if t not in not_text
            ]
