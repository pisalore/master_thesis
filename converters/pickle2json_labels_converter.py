...
def generate_json_labels(pickle_file, png_path):
    ...
    for paper_idx, paper_key in enumerate(docs_instances):
        ...
        for page_idx, page_img_path in enumerate(
            Path(png_path).rglob(f"{paper_key}_*.png")
        ):
            ...
            if page_idx == 0:
                ...
                if abstract:
                    json_annotations[key]["annotations"].append(
                        {
                            "segmentation": calculate_segmentation(
                                abstract.get("coords")
                            ),
            ...
            for category in docs_instances.get(paper_key).keys():
                ...
                if category not in ["title", "abstract"]:
                    ...
                    ann_object = (
                        docs_instances.get(paper_key).get(category).get(page_idx + 1)
                    )
                    if ann_object:
